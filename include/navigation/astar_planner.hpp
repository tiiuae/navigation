...
public:
  AstarPlanner(double safe_obstacle_distance, double euclidean_distance_cutoff, double planning_tree_resolution, double distance_penalty, double greedy_penalty,
               double vertical_penalty, bool unknown_is_occupied, float navigation_tolerance, double max_waypoint_distance, double planning_timeout);
...
public:
  std::vector<octomap::point3d> findPath(const octomap::point3d &start, const octomap::point3d &goal, std::shared_ptr<octomap::OcTree> mapping_tree,
                                         ...
                                         std::function<void(const std::set<Expansion, CostComparator> &, const octomap::OcTree &)> visualizeExpansions,
                                         bool                                                                                      visualize);
  ...
  void generatePlanningTree(std::shared_ptr<octomap::OcTree> tree, const octomap::point3d &start, double resolution);
...
private:
   ...
   *
   * @return true if the line only contains free cells and does not exceed the given threshold
   */
