...
private:
  ...
  std::vector<octomap::point3d> waypoint_buffer_;
  ...
  // services provided
  rclcpp::Service<fog_msgs::srv::Vec4>::SharedPtr goto_service_;

  // service callbacks
  bool gotoCallback(const std::shared_ptr<fog_msgs::srv::Vec4::Request> request, std::shared_ptr<fog_msgs::srv::Vec4::Response> response);

  bool                goalReached(const octomap::point3d &goal, double dist_tolerance);
  nav_msgs::msg::Path waypointsToPathMsg(std::vector<octomap::point3d> waypoints);
  ...
  void visualizeGoals(const octomap::point3d goal, const octomap::point3d temporary_goal);
...
Navigation::Navigation(rclcpp::NodeOptions options) : Node("navigation", options) {
  ...
  waypoints_publisher_   = this->create_publisher<nav_msgs::msg::Path>("~/waypoints_out", 10);
  ...
  // service handlers
  goto_service_ = this->create_service<fog_msgs::srv::Vec4>("~/goto_in", std::bind(&Navigation::gotoCallback, this, _1, _2));
...
/* gotoCallback //{ */
bool Navigation::gotoCallback([[maybe_unused]] const std::shared_ptr<fog_msgs::srv::Vec4::Request> request,
  ...
  if (request->goal[2] < min_altitude_) {
    RCLCPP_WARN(this->get_logger(), "[%s]: Goal is below the minimum allowed altitude!", this->get_name());
    RCLCPP_INFO(this->get_logger(), "[%s]: Goal shifted to altitude %.2f", this->get_name(), min_altitude_);
    ;
  }
  ...
  if (is_initialized_ && getting_octomap_ && getting_odometry_) {
    ...
    switch (status_) {
      ...
      case PLANNING: {
        ...
        if (goalReached(goal_, navigation_tolerance_)) {
          RCLCPP_INFO(this->get_logger(), "[%s]: Final goal reached!", this->get_name());
          status_ = IDLE;
          break;
        }
        ...
        visualizeGoals(goal_, current_goal_);

        if (octree_ == NULL || octree_->size() < 1) {
          RCLCPP_WARN(this->get_logger(), "[%s]: Octomap is NULL or empty! Abort mission.", this->get_name());
          status_ = IDLE;
          break;
        }
        ...
        if (goal_in_map && goal_valid) {
          ...
          auto waypoints = planner->findPath(uav_pos_, current_goal_, octree_, std::bind(&Navigation::visualizeTree, this, _1),
                                             std::bind(&Navigation::visualizeExpansions, this, _1, _2), visualize_planner_);
          if (waypoints.size() < 2) {
            RCLCPP_ERROR(this->get_logger(), "[%s]: Path not found! Likely reason: bowl-shaped obstacle larger than detection radius", this->get_name());
            status_ = IDLE;
            break;
          }
          ...
          double dist = 0.0;
          waypoint_buffer_.push_back(waypoints[0]);
          for (size_t i = 1; i < waypoints.size(); i++) {
            waypoint_buffer_.push_back(waypoints[i]);
            dist += AstarPlanner::distEuclidean(waypoints[i - 1], waypoints[i]);
            if (dist > replanning_distance_) {
              break;
            }
          }
          visualizePath(waypoints);
          auto waypoints_msg = waypointsToPathMsg(waypoint_buffer_);
          waypoints_publisher_->publish(waypoints_msg);
          current_goal_ = waypoint_buffer_.front();
          status_       = MOVING;
          break;
        }
        ...
        if (goal_in_map && !goal_valid) {
          ...
          current_goal_.x() = new_goal.x();
          current_goal_.y() = new_goal.y();
          current_goal_.z() = new_goal.z();

          goal_.x() = new_goal.x();
          goal_.y() = new_goal.y();
          goal_.z() = new_goal.z();
          visualizeGoals(goal_, current_goal_);
          // goal is now valid -> planning will be executed in the next cycle
          break;
        }

        if (!goal_in_map) {
          RCLCPP_INFO(this->get_logger(), "[%s]: Goal is outside of map", this->get_name());
          auto temporary_goal = planner->generateTemporaryGoal(uav_pos_, current_goal_);
          auto new_goal       = temporary_goal.first;
          priorize_vertical_  = temporary_goal.second;
          RCLCPP_INFO(this->get_logger(), "[%s]: Generated a temporary goal", this->get_name());
          current_goal_.x() = new_goal.x();
          current_goal_.y() = new_goal.y();
          current_goal_.z() = new_goal.z();
          visualizeGoals(goal_, current_goal_);
          if (priorize_vertical_) {
            ...
            waypoints.push_back(current_goal_);
            auto waypoints_msg = waypointsToPathMsg(waypoints);
            waypoints_publisher_->publish(waypoints_msg);
            status_ = MOVING;
          }
          // goal is now in map -> planning will be executed in the next cycle
          break;
        }
        break;
      }
      ...
      case MOVING: {
        if (goalReached(current_goal_, navigation_tolerance_)) {
          if (waypoint_buffer_.empty()) {
            ...
            current_goal_.x() = goal_.x();
            current_goal_.y() = goal_.y();
            current_goal_.z() = goal_.z();
            status_           = PLANNING;
          } else {
            RCLCPP_INFO(this->get_logger(), "[%s]: Moving to next waypoint", this->get_name());
...
/* goalReached //{ */
bool Navigation::goalReached(const octomap::point3d &goal, double dist_tolerance) {
...
/* waypointsToPathMsg //{ */
nav_msgs::msg::Path Navigation::waypointsToPathMsg(const std::vector<octomap::point3d> waypoints) {
...
/* visualizeGoals //{ */
void Navigation::visualizeGoals(const octomap::point3d goal, const octomap::point3d temporary_goal) {
  ...
  p.x                         = temporary_goal.x();
  p.y                         = temporary_goal.y();
  p.z                         = temporary_goal.z();
  msg.points.push_back(p);
  msg.colors.push_back(c);

  // final goal
  c   = generateColor(0.2, 1.0, 0.2, 1.0);
  p.x = goal.x();
  p.y = goal.y();
  p.z = goal.z();
  msg.points.push_back(p);
